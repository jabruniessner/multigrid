...
int main(int argc, char *argv[]) {
  ...
  for (int stage = 0; stage <= num_stages; stage++)
    for (int i = 14; i < 22; i++) {
      ...
      point =
          point * (-1) *
              ((radius - stage / (num_stages * std::sqrt(3))) / norm(point)) +
          center -
          volume_computer::vector3d{
              static_cast<DataType>(std::signbit(-point[0])),
              static_cast<DataType>(std::signbit(-point[1])),
  ...
  for (int stage = 0; stage < num_stages; stage++) {
    ...
    for (int i = 0; i < 3; i++)
      for (int j = -1; j < 2; j += 2)
        for (int k = -1; k < 2; k += 2) {
          auto &point = points_span[stage, cube_number++];
          point[i] = 0;
          point[(i + 1) % 3] = j;
          point[(i + 2) % 3] = k;
  ...
  for (int i = 1; i < 34; i++) {
    for (int stage = 0; stage < num_stages; stage++) {
      ...
      auto &pointy = points_span[stage, i];
      ...
      std::span<std::uint32_t, 19> tet_grid_span(&cube_edges[stage, i, 0], 19);
      std::span<DataType, 6> tetrahedra_span(&volume_tetrahedra[stage, i, 0],
                                             6);
      ...
      std::size_t position_0 =
                      static_cast<std::size_t>(points_span[stage, i][0]),
                  position_1 =
                      static_cast<std::size_t>(points_span[stage, i][1]),
                  position_2 =
                      static_cast<std::size_t>(points_span[stage, i][2]);
