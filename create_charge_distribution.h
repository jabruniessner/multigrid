...
void add_charges_to_distribution(domain::Domain<Dim, Strides...> domain,
  ...
  for (int i = 0; i < Dim; i++) {
    ...
    const DataType distance_lower =
        (Position_grid_spacing - lower) * grid_spacing;

    const DataType distance_upper =
        (lower + 1 - Position_grid_spacing) * grid_spacing;

    distances[2 * i] = distance_lower;
    distances[2 * i + 1] = distance_upper;
  }
