...
public:
  ...
  void buildmultilevelops() {
    if constexpr (level == 1) {
      ...
      buildmultilevelops<vanilla_prolongation, level - 1>();
    } else {
...
#undef GET_VALUES_BUF
  ...
  void v_cycle() {
    if constexpr (level == 1) {
      ...
      //        sol.template get_domain<level>(), sol2.template
      //        get_domain<level>(), rhs_domain.template get_domain<level>(),
      //        get_map<level>());
