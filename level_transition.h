...
void coarsening(Domain<Dim, ((strides_all+1)/2-1)...>& dest, Domain<Dim, strides_all...>& src, const std::array<DataType, size> values, const std::array<Offsets, size> offsets, std::index_sequence<dims...>)
{
	assert(dest.q == src.q);
...
void coarsening(Domain<Dim, ((strides_all+1)/2-1)...>& dest, Domain<Dim, strides_all...>& src, const std::array<DataType, size> values, const std::array<Offsets, size> offsets)
{
...
void refinement(Domain<Dim, strides_all...>& dest, Domain<Dim, ((strides_all+1)/2-1)...>& src, std::index_sequence<dims...>)
{
	...
	dest.q.submit([&](sycl::handler& h){
		h.parallel_for(sycl::range<Dim>(dest.strides[dims]...), [=](sycl::id<Dim> I){
				...
				dest(I[dims]...) = domain_refinement_helper(src, empty_index_tuple, I[dims]...);
		});
	});
...
void refinement(Domain<Dim, strides_all...>& dest, Domain<Dim, ((strides_all+1)/2-1)...>& src)
{
