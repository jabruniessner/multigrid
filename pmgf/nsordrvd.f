...
c* @brief   Driver for nonlinear SOR iteration.
c* @version $Id: nsordrvd.f,v 1.2 2006/03/24 00:58:51 mholst Exp $
c* @attention
c* @verbatim
c*
...
c* ///////////////////////////////////////////////////////////////////////////
      ...
      subroutine nsordriv(iparm,rparm,iwork,rwork,u,
     2   xf,yf,zf,gxcf,gycf,gzcf,a1cf,a2cf,a3cf,ccf,fcf,tcf)
c* *********************************************************************
c* purpose:
c*
c*    driver for linear/nonlinear sor iteration.
c*
c* author:  michael holst
c* *********************************************************************
      implicit         none
...
c*    *** call the multigrid driver ***
      call nsordriv2(iparm,rparm,nx,ny,nz,u,iwork(k_iz),
     2   rwork(k_w0),
...
c*    *** return and end ***
      ...
      subroutine nsordriv2(iparm,rparm,nx,ny,nz,u,iz,
     2   w0,
     3   ipc,rpc,ac,cc,fc,
...
c*    *** MATLAB ***
     ...
     2   'sor_st',(tsetupf+tsetupc),'sor_so',tsolve
...
c*    *** return and end ***
      ...
      subroutine nsorgo(nx,ny,nz,x,r,w1,w2,
     2   istop,itmax,iters,ierror,
...
c*    *** do some i/o if requested ***
      if (iinfo.ne.0) then
         write(6,100)'% NSORGO: starting: ',nx,ny,nz
...
c*    *** return and end ***
 99   continue
      ...
      subroutine sorgo(nx,ny,nz,x,r,w1,w2,
     2   istop,itmax,iters,ierror,
     3   iok,iinfo,epsiln,errtol,omega,
     4   ipc,rpc,ac,cc,fc,tru)
c* *********************************************************************
...
c*    *** do some i/o if requested ***
      if (iinfo.ne.0) then
         write(6,100)'% SORGO: starting:  ',nx,ny,nz
 100     format(a,(2x,' [',i3,',',i3,',',i3,'] '))
      endif
c*
c*    *** initial wall clock ***
