...
c* @version $Id: maind.f,v 1.2 2006/03/24 00:58:51 mholst Exp $
c* @attention
c* @verbatim
c*
...
c***********************************************************************
c* ***parameter        (ngrid=97,  nrwk=9960712,   niwk=2000)
...
c*    *** other parameters ***
      ...
      parameter        (nxm=ngrid,nym=ngrid,nzm=ngrid)
      parameter        (nf=nxm*nym*nzm,ifudge=100,nfarr=nf+nf/6+ifudge)
c*
c*    *** main storage *** 
      integer          nx,ny,nz,key,meth,nlev,mgcoar,mgdisc,mgsolv
      integer          nrwk_tot,niwk_tot
      integer          iparm(100), iwork(niwk)
      double precision rparm(100), rwork(nrwk)
      double precision a1cf (nfarr)
      double precision a2cf (nfarr)
      double precision a3cf (nfarr)
      double precision ccf  (nfarr)
      double precision fcf  (nfarr)
      double precision tcf  (nfarr)
      double precision u    (nfarr)
      double precision xf   (nxm,5)
      double precision yf   (nym,5)
      double precision zf   (nzm,5)
      double precision gxcf (nym,nzm,10)
      double precision gycf (nxm,nzm,10)
      double precision gzcf (nxm,nym,10)
...
c*    *** print out the required sizes ***
      ...
      mgdisc = iparm(19)
      mgsolv = iparm(21)
      call mgsize(mgcoar,mgdisc,mgsolv,nx,ny,nz,nlev)
      if ((nx.gt.nxm).or.(ny.gt.nym).or.(nz.gt.nzm)) then
         print*,'% MGMAIN:  (nx,ny,nz) too large...'
         goto 99
      endif
c*
...
c*    *** call linear solvers ***
      ...
      if (meth .eq. 0) then
         ...
         call cgmgdriv (iparm,rparm,iwork,rwork,u,
     2      xf,yf,zf,gxcf,gycf,gzcf,a1cf,a2cf,a3cf,ccf,fcf,tcf)
     ...
     2      xf,yf,zf,gxcf,gycf,gzcf,a1cf,a2cf,a3cf,ccf,fcf,tcf)
      elseif (meth .eq. 3) then
         ...
         call ncghsdriv (iparm,rparm,iwork,rwork,u,
     2      xf,yf,zf,gxcf,gycf,gzcf,a1cf,a2cf,a3cf,ccf,fcf,tcf)
