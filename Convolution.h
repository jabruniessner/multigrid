...
template <typename DataType, typename Offsets, size_t size, Dimension Dim,
          Length... strides_all, std::size_t... dims>
DataType PBE_Convolve_kernel(Domain<Dim, strides_all...> &src,
                             ...
                             const std::array<DataType, size> &values,
                             const std::array<Offsets, size> &offsets,
                             sycl::id<Dim> I, std::index_sequence<dims...>) {
...
DataType PBE_Convolve_kernel(
    Domain<Dim, strides_all...> &src, Domain<Dim, strides_all...> &kappa_map,
    Domain<Dim, strides_all...> &epsilon_map, const DataType &kappa_2,
    const DataType grid_step, const DataType epsilon_r,
    const DataType delta_epsilon, const std::array<DataType, size> &values,
    const std::array<Offsets, size> &offsets, sycl::id<Dim> I) {
